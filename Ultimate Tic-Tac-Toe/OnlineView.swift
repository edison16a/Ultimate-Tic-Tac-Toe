...
struct GameView2: View {
    ...
    private func checkForWin(_ bigIndex: Int) {
        let sub = board[bigIndex]
        if isWinningCombo(sub[0], sub[1], sub[2]) ||
           isWinningCombo(sub[3], sub[4], sub[5]) ||
           isWinningCombo(sub[6], sub[7], sub[8]) ||
           isWinningCombo(sub[0], sub[3], sub[6]) ||
           isWinningCombo(sub[1], sub[4], sub[7]) ||
           isWinningCombo(sub[2], sub[5], sub[8]) ||
           isWinningCombo(sub[0], sub[4], sub[8]) ||
           isWinningCombo(sub[2], sub[4], sub[6]) {
            mainBoard[bigIndex] = activePlayer
        }
    }

    private func isWinningCombo(_ a: String, _ b: String, _ c: String) -> Bool {
        return !a.isEmpty && a == b && b == c
    }
                    ...
                    HStack(spacing: 2) {
                        ForEach(0..<3) { col in
                            ...
                            Text(subBoard[index])
                                .font(.title)
                                .fontWeight(.bold)
                                .foregroundColor(subBoard[index] == "X" ? Color.blue.opacity(1) : (subBoard[index] == "O" ? Color.red.opacity(1) : .black))
    ...
    var body: some View {
        VStack {
            ...
            if matchmaking {
                ...
                GameView2(isBot: true)
                        .onDisappear {
                            inMatch = false // Reset match state when leaving
                        }
            } else {
